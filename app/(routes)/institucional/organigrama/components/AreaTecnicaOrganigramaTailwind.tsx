...
const OrganigramaBox: React.FC<{
  ...
  const getBoxClass = () => {
    ...
    } else if (item.boxClass === 'dept-box-special') {
      return `${baseClass} bg-[#F5F5F5]`; // Color azul claro para el departamento especial
    } else if (item.boxClass === 'section-box') {
      return `${baseClass} bg-[#E0F7FA]`;
    } else if (item.boxClass === 'section-box-special') {
      return `${baseClass} bg-[#E0F7FA]`; // Color azul claro para la sección especial
    }
...
const AreaTecnicaOrganigramaTailwind: React.FC = () => {
  ...
  return (
    <div className="organigrama-container relative w-full p-4">
      ...
      {/* Departamentos */}
      <div className="departments-section flex justify-center space-x-4 mb-10">
      ...
      <div className="sections-container flex justify-center space-x-4">
          ...
          {deptSections.dept1.length > 0 && (
            <div className="flex justify-center space-x-2">
              {deptSections.dept1.map(item => (
                <div key={item.id} className="w-full h-26">
                  <OrganigramaBox item={item} />
                </div>
              ))}
            </div>
        ...
        <div className="dept4-sections w-full max-w-xs flex flex-col space-y-4">
          {deptSections.dept4.length > 0 && (
            <div className="flex justify-center space-x-2">
              {deptSections.dept4.map(item => (
                <div key={item.id} className="w-full h-16">
                  <OrganigramaBox item={item} />
                </div>
              ))}
